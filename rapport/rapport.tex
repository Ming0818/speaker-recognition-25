...
Pour choisir les paramètres à ajuster dans les différents algorithmes (la constante $C$ la plus adaptée pour la technique de la SGD (\ref{sgd})), nous avons utilisé le protocole suivant :
\begin{itemize}
...
En ce qui concerne l'optimisation quadratique avec le noyau de Fisher, nous avons lancé des tests sur plusieurs valeurs des paramètres à ajuster. Nous avons observé que le paramètre $C$ n'influençait pas les résulats donc nous montrons seulement l'influence du nombre de gaussiennes sur la figure \ref{opt_qp}. Le nombre de gaussiennes optimal est autour de 30 et nous prendrons donc cette valeur par la suite.
